...
resource "google_compute_subnetwork" "trust-sub" {
  name          = var.vpc_trust_subnet_name
  ip_cidr_range = var.vpc_trust_subnet_cidr
  network       = google_compute_network.trust-net.self_link
  region        = var.vpc_region
}

resource "google_compute_network" "trust-net" {
  name                    = var.vpc_trust_network_name
  auto_create_subnetworks = "false"
}
...
resource "google_compute_firewall" "trust-allow-outbound" {
  name    = "trust-allow-outbound"
  network = google_compute_network.trust-net.name
